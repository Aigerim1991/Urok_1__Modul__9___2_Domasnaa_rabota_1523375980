...
void printInfoProduct(Product  * pr, int len)
{
	...
	for (int i = 0; i < len; i++)
	{
		printf("#%2d. %12s: id %d, Количествo	 %d,Цена %d, Год изготовления %d,Производитель %s\n",
			i + 1,
...
void printInfoComp(Computer  * cmp, int len)
{
	...
	for (int i = 0; i < len; i++)
	{
		printf("#%2d. %12s: Название  %d, RAM %d, DVD ROM %d, Стоимость %d\n",
			i + 1,
			cmp[i].cmpMkr.mkr,
			cmp[i].frequency,
			cmp[i].RAM,
...
void printInfoStd(Student * students, int len)
{
	...
	for (int i = 0; i < len; i++)
	{
		printf("#%d.Имя: %s, Группа %s,	Год рождения: %d.%d.%d;\n",
			i + 1,
			students[i].name,
			students[i].groupe,
			students[i].bdt.day,
...
void generatePosition(ocupation * pos)
{
	...
	{
	case 1:
		strcpy(pos->ocup, "Èíæåíåð");
		break;

	case 2:
		strcpy(pos->ocup, "Òåõíèê");
		break;

	case 3:
		strcpy(pos->ocup, "Ðàáî÷èé");
		break;
	}
}
